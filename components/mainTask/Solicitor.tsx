...
export function SolicitorCard({ contact }: { contact: ContactData }) {
  const [opened, { toggle }] = useDisclosure(true);

  const { dispatch } = useAppContext();

  const handleContactType = () => {
    dispatch({ type: 'SET_CONTACT_TYPE', payload: 'solicitor' });
  ...
  return (
    <Card withBorder mt="sm" radius="md" padding="sm" bg="var(--mantine-color-body)">
      <Flex justify="space-between">
        <Box className={classes.titles} onClick={handleContactType} style={{ cursor: 'pointer' }}>
          <Text
        ...
        {contact ? (
          <Box mb="md" key={contact?.contact_id}>
            <Link className={classes.relatedPartyLink} href="" style={{ textDecoration: 'none' }}>
              <Title order={6}>{`${contact.first_name} ${contact.last_name}`}</Title>
            </Link>
            <Flex gap="sm">
