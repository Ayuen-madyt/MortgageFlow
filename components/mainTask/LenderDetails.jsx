...
export default function LenderDetails({ main_task_id }) {
  ...
  const { data: lender } = useSWR([`/lender/get/${lenderId}`, token], ([url, token]) =>
    fetcher(url, token)
  );

  const { mutate: mutateTask } = useSWR([`/main/task/get/${main_task_id}`, token], ([url, token]) =>
    fetcher(url, token)
  );
  ...
  const saveLender = () => {
    setLoading(!loading);
    axiosInstance
      .put(`/lender/update/${lenderId}`, lenderDetails)
      .then((response) => {
        setLoading(false);
  ...
  const updateMainTaskLender = (lender_id) => {
    ...
    axiosInstance
      .put(`/main/task/update/${main_task_id}`, { lender_id: parseInt(lender_id) })
      .then((res) => {
        mutateTask(`/main/task/get/${main_task_id}`);
        notifications.show({
          title: 'Success',
  ...
  return (
    <Box>
      <Flex justify="flex-start" align="center" style={{ color: '#0275d8' }}>
        ...
        />
        <Text onClick={handleShowTabsClick} style={{ cursor: 'pointer' }} size="sm">
          Go back
        </Text>
      </Flex>
      <Card
          ...
          <Box mt="md">
            <Flex onClick={toggleLinkedBranch} justify="space-between">
              <Box className={classes.titles} style={{ cursor: 'pointer' }}>
                ...
                <Text size="xs" c="dimmed">
                  If you have a relationship with a branch that you refer clients to, enter details
                  here
                </Text>
              </Box>
              <IconChevronRight
          ...
          <Box mt="md">
            ...
            <Collapse in={openDischarges} mt="xs">
              ...
              <PhoneInput
                ...
                onChange={(event) => setLenderDetails({ ...lenderDetails, discharges_fax: value })}
              />
              <TextInput
                leftSectionPointerEvents="none"
