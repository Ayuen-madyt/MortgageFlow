...
export function BuyersAgentCard({ contact }: { contact: ContactData }) {
  const [opened, { toggle }] = useDisclosure(true);

  const { dispatch } = useAppContext();

  const handleContactType = () => {
    dispatch({ type: 'SET_CONTACT_TYPE', payload: 'buyers_agent' });
  ...
  return (
    <Card withBorder mt="sm" radius="md" padding="sm" bg="var(--mantine-color-body)">
      <Flex justify="space-between" style={{ cursor: 'pointer' }}>
        <Box onClick={handleContactType} className={classes.titles}>
          <Text
